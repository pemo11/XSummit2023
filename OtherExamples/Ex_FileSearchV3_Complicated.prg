// file: Ex_FileSearchV3_Complicated.prg
// The final version with Cancel but to complicated
...
Class MainForm Inherit Form
    ...
    Async Method ListDirectory(DirPath As String, Filter As String) As Task<List<String>>
        Return await Task.Run({=> Directory.EnumerateFiles(DirPath, Filter):ToList()})
    
    Async Method ListDirectories(DirPath As String, Filter As String, lb As Listbox, Token As CancellationToken) As Task
        var files := await ListDirectory(DirPath, Filter)
        lb:Items:AddRange(files:ToArray())
         ForEach var subdir in Directory.EnumerateDirectories(DirPath)
           files := await ListDirectory(subdir, Filter)
           lb:Items:AddRange(files:ToArray())
         Next
        
    Async Method ListDirectoryFiles(dirInfo As DirectoryInfo, FileFilter As String, Token As CancellationToken) As Task<List<FileInfo>>
         ...
         Return await Task.Run(async Delegate {
           Try
             ...
             ForEach var dir in dirInfo:GetDirectories()
              ...
              dirFiles:AddRange(Await ListDirectoryFiles(subDirInfo, FileFilter, Token))
    ...
    Async Method FileSearch(StartDirectory As String, FileFilter As String, Token As CancellationToken) As Task<List<FileInfo>>
      Var dirInfo := DirectoryInfo{StartDirectory}
      Var dirFiles := Await ListDirectoryFiles(dirInfo, FileFilter, Token)
      Return dirFiles
    ...
    Async Method btnStart_Click(Sender As Object, e As EventArgs) As Void
        ...
        await FileSearch(txtDirectory:Text, txtFilter:Text, cts:Token)
        btnStart:Enabled := True
        btnCancel:Enabled := False
